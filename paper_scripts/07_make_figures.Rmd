...
make_specks_fig <- function(df, level_order, group_colors, type){
  ...
  df$type <- ordered(df$type, levels = c("Logistic Regression", "LASSO", "Random Forest", "Regression Tree"))
...
group_colors <- c("Logistic Regression" ="#361052", "LASSO" = "red", "Regression Tree" = "green", "Random Forest" = "blue")
...
plot_pov <- ggplot(plot, aes(x = synth_type, y = pmse, color = BLACK_class))+
  geom_point(alpha = .5)+
  ylab("log(pMSE-Ratio)")+
  xlab("Synthesis Type")+
  theme_classic()+
