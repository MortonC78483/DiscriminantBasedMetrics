...
**EM: This is almost exactly like the quicksynth() function I made haha!**
...
**EM: I haven't seen this approach before, and I'm wondering if this is something you came up with (which is really cool) or if you've read about this somewhere. If it's the latter, I'd love to read where you got this idea from! I also noticed that you did not synthesize the poverty variable, and I'm just wondering what your rationale behind that was for clarity, Also, thinking back to my project, is there a reason you used the variable order you did? Do you think that other orderings, specifically orderings that favor variables correlated to poverty status, would produce different results?**
...
**EM: Just to make sure I understand, the results show that when using a discriminator to distinguish between the synthetic and confidential data using a pair of variables, the pairs including total family income had the best performing models for the "bad" synthesis (i.e., the synthesis where everyone was from the majority class)? If so, that makes sense to me assuming the family income values for just the minority class are less varied than the values with both the majority and minority classes. I wonder if you would see the same thing the income values came from the same distribution for each class (I'm thinking you wouldn't, which begs the question if you could pick out potential "problem" variables based on their association with the imbalanced sensitive variable).**

**EM: I'm also not too familiar on issues with using only categorical variables as inputs for a discriminator. In my mind, of the distribution of the categorical variables is different for two types of observations, then a discriminator could easily discriminate between them. I'm guessing this isn't always the case?**

**EM: Final note, but I'm having a hard time understanding what I would do with this information. Is this identifying variables at higher risks of retaining less utility if a synthesis does not take into account all of the values of an imbalanced sensitive variable? Also, how is the "bad" synthesis example related to syntheses in practice?**
