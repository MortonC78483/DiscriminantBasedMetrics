...
auc_all$pmse$.pmse_ratio[2]
auc_maj$pmse$.pmse_ratio[2]
```
...
min_pmse_pov_all = d_tree_min$pmse$.pmse[2]
min_pmse_ratio_pov_all = d_tree_min$pmse$.pmse_ratio[2]
...
maj_pmse_pov_all = d_tree_maj$pmse$.pmse[2]
maj_pmse_ratio_pov_all = d_tree_maj$pmse$.pmse_ratio[2]

c(min_auc_pov_all, maj_auc_pov_all)
c(min_pmse_pov_all, maj_pmse_pov_all)
c(min_pmse_ratio_pov_all, maj_pmse_ratio_pov_all)
```

```{r}
# attempt to calculate null pMSE baseline for when synthetic +conf data are size of minority group, but have data from majority group
conf_min_from_maj <- conf_maj[sample(1:nrow(conf_maj), size = nrow(conf_min), replace = F),]
synth_min_from_maj <- synth_maj[sample(1:nrow(synth_maj), size = nrow(synth_min), replace = F),]

d_min_from_maj <- discrimination(synth_min_from_maj, conf_min_from_maj)
d_tree_min_from_maj <- calc_disc(d_min_from_maj)
d_tree_min_from_maj <- d_tree_min_from_maj %>%
  add_discriminator_auc() %>%
  add_pmse() %>%
  add_pmse_ratio(times = 10)
min_from_maj_auc_pov = d_tree_min_from_maj$discriminator_auc$.estimate[2]
min_from_maj_pmse_pov = d_tree_min_from_maj$pmse$.pmse[2]
min_from_maj_pmse_ratio_pov = d_tree_min_from_maj$pmse$.pmse_ratio[2]
  
# find what the null was
min_from_maj_pmse_pov/min_from_maj_pmse_ratio_pov
maj_pmse_pov_all/maj_pmse_ratio_pov_all
min_pmse_pov_all/min_pmse_ratio_pov_all
```
...
for (i in 1:N){
  ...
  min_pmse = append(min_pmse, d_tree_min$pmse$.pmse[2])
  min_pmse_ratio = append(min_pmse_ratio, d_tree_min$pmse$.pmse_ratio[2])
  # isolate to majority group
  d_maj <- discrimination(synth_maj, conf_maj)
  d_tree_maj <- calc_disc(d_maj)
  d_tree_maj <- d_tree_maj %>%
  ...
  maj_pmse = append(maj_pmse, d_tree_maj$pmse$.pmse[2])
  maj_pmse_ratio = append(maj_pmse_ratio, d_tree_maj$pmse$.pmse_ratio[2])
}

print("raw values")
maj_pmse_ratio_pov_all
min_pmse_ratio_pov_all
min_pmse_ratio
maj_pmse_ratio

print("Subtraction")
maj_pmse_ratio_pov_all-min_pmse_ratio_pov_all
maj_pmse_ratio-min_pmse_ratio
